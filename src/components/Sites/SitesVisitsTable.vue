            ...
            <table class="w-full mt-3" v-if="visits.length >= 1">
                <thead class="bg-primary-light border border-primary-light">
                    <tr class="[&>th]:text-left [&>th]:p-1 [&>th]:px-2 border-primary-light">
                        <th class="w-1">Id</th>
                        <!-- erreur dans la condition : kle if est exécuté avant le for  -->
                        <th v-for="displayVisit in displayVisits" v-if="displayVisit.display == true"><span>{{ displayVisit.name }}</span></th>
                        <th>Site</th>
                        <th>Page</th>
                        <th></th>
                ...
                <tbody>
                    <VisitsRow v-for="visit in visits" class="[&:nth-child(2n)]:bg-gray-light" :key="visit.id" :visit="visit"/>
                </tbody>
            </table>
            <p class="mt-3" v-else>
